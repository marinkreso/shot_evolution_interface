    ...
    with ui.header().classes('bg-black text-white p-4 shadow-md'):
        with ui.row().classes('items-center justify-between w-full'):
            ...
            with ui.row().classes('mx-auto'):
                ...
                tec_players = ['MARTINEZ', 'CARRENO BUSTA']
                selection = ui.select(tec_players, value=prefix, on_change=lambda e: ui.navigate.to(f'https://gsapostmatch.azurewebsites.net/tec/match_new_list_tec/{e.value}')).props(f'color=bg-gray-800').style('font-size: 120%;').props('rounded outlined')
                
                #videoplatform = ui.button('Video Platform', on_click=lambda: ui.navigate.to(f'https://gsapostmatch.azurewebsites.net/tec/shots_view_tec/{prefix}')).props(f'color=bg-gray-800').style('font-size: 120%')#; text-decoration:underline')#.classes('text-lg')
                #videoplatform = ui.button('Video Platform', on_click=lambda: ui.navigate.to(f'https://client.goldensetanalytics.com/Video')).props(f'color=bg-gray-800').style('font-size: 120%')#; text-decoration:underline')#.classes('text-lg')
                
                #selection = ui.select(on_players, value=prefix, on_change=lambda e: ui.navigate.to(f'https://gsapostmatch.azurewebsites.net/tec/match_new_list_tec/{e.value}'))
                
                postmatch = ui.button('Post Match Reports', on_click=lambda: ui.navigate.to(f'https://gsapostmatch.azurewebsites.net/tec/match_new_list_tec/{prefix}')).props(f'color=bg-gray-800').style('font-size: 120%;')#.classes('text-lg')
                tracking = ui.button('Match Evolution Tracker', on_click=lambda: ui.navigate.to(f'https://gsapostmatch.azurewebsites.net/tec/tracking_tec/{prefix}')).props(f'color=bg-gray-800').style('font-size: 120%;')#.classes('text-lg')
                shot_evolution = ui.button('Shot Evolution Tool', on_click=lambda: ui.navigate.to(f'https://gsapostmatch.azurewebsites.net/tec/shot_evolution_interface_tec/{prefix}')).props(f'color=bg-gray-800').style('font-size: 120%;')#.classes('text-lg')
                movement_tool = ui.button('Movement Tool', on_click=lambda: ui.navigate.to(f'https://gsapostmatch.azurewebsites.net/tec/movement_report_tec/{prefix}')).props(f'color=bg-gray-800').style('font-size: 120%;')#.classes('text-lg')
...
async def insert_data_and_generate_link(nor, prefix, results):  
    ...
    if all(player for player in nor.players):
        ...
        for i in range(nor.number):
            ...
            if nor.matches[i]:
                data.append({
                    ...
                    'matches': [x for x in nor.matches[i] if x in nor.filter_tecly_matches[i]],
                    'name': column_name.upper()
                })
            else:
                data.append({
                    'player': nor.players[i],
                    'matches': nor.filter_tecly_matches[i],
                    'name': column_name.upper()
                })
        ...
        ui.navigate.to(f'https://gsapostmatch.azurewebsites.net/tec/report_tec2/{prefix}_{link_id}')
        #ui.navigate.to(f'http://127.0.0.1:8000/tec/report_tec2/{prefix}_{link_id}')
    else:
                    ...
                    for col_spec in columns:
                        ...
                        if cls == ui.label or cls == ui.link:
                            if cls == ui.label:
                                ...
                                cls_parms["text"] = 'PLAYER PAGE LINK'
                                cls_parms["target"] = row[col_name]
                        else:
                            cls_parms["value"] = row[col_name]  # avoid triggering on_change when binding
...
def init(fastapi_app: FastAPI) -> None:
    ...
    ui.run_with(
        fastapi_app,
        mount_path='/tec',  # NOTE this can be omitted if you want the paths passed to @ui.page to be at the root
    ...
    @ui.page('/tracking_tec/{prefix}', dark=True,favicon='https://operationslakedb.blob.core.windows.net/gsa-post-match/fav.png')
    def tracking(prefix):
    ...
    @ui.page('/match_new_list_tec2/{prefix}',favicon='https://operationslakedb.blob.core.windows.net/gsa-post-match/fav.png')
    def match_new_list_tec2(prefix):
        ...
        #style="position: relative; top: -10vh; height: 110vh; width: 100%; border: none;"></iframe>
    @ui.page('/points_view_tec/{prefix}',favicon='https://operationslakedb.blob.core.windows.net/gsa-post-match/fav.png')
    def points_view_tec(prefix):
        create_text_header(prefix)
        ui.add_head_html('<style>body {background-color: #081424; }</style>')
        #ui.html('''<iframe src="https://app.goldensetanalytics.com/shotsview" width="100%" height="1000px" style="clip-path: inset(10% 0 0 0); border:none;"></iframe>''').classes('w-full')
        ui.html('''<iframe src="https://client.goldensetanalytics.com/Video" width="100%" height="1000px" style="position: relative; top: -4vh; height: 110vh; width: 100%; border:none;"></iframe>''').classes('w-full')
        #style="position: relative; top: -10vh; height: 110vh; width: 100%; border: none;"></iframe>
    @ui.page('/shots_view_tec/{prefix}',favicon='https://operationslakedb.blob.core.windows.net/gsa-post-match/fav.png')
    def shots_view_tec(prefix):
        create_text_header(prefix, 'videoplatform')
        ui.add_head_html('<style>body {background-color: #081424; }</style>')
        ui.html('''<iframe src="https://app.goldensetanalytics.com/shotsview" width="100%" height="1000px" style="clip-path: inset(0 0 0 0); border:none;"></iframe>''').classes('w-full')
    ...
    @ui.page('/report_tec2/{prefix}_{keyx}',favicon='https://operationslakedb.blob.core.windows.net/gsa-post-match/fav.png')
    def report_tec2(prefix, keyx):
        create_text_header(prefix)
        ui.add_head_html('<style>body {background-color: #081424; }</style>')
        #ui.html('''<iframe src="https://app.goldensetanalytics.com/shotsview" width="100%" height="1000px" style="clip-path: inset(10% 0 0 0); border:none;"></iframe>''').classes('w-full')
    ...
    @ui.page('/shot_evolution_interface_tec/{prefix}', response_timeout=15, dark=True,favicon='https://operationslakedb.blob.core.windows.net/gsa-post-match/fav.png')
...
<header class="bg-[url(/images/footer.jpg)] w-full relative">
    ...
    <div class="container relative mx-auto flex items-center content-center align-middle h-full min-h-[200px] mt-10">
        ...
        def remove_column(index):
            ...
            del nor.filter_tecly_matches[index]
            del nor.years[index]
        ...
        def add_new_column(e):
            nor.number = nor.number + 1
            nor.matches.append([])
            nor.filter_tecly_matches.append([])
            nor.years.append([])
        ...
        async def form_view(x):
            ...
            async def on_company_change(i, dialoger):
                ...
                nor.filter_tecly_matches[i] = selected_matches
                rows = [
                                {
        ...
        ui.space()
        ui.space()
    @ui.page('/match_new_list_tec/{prefix}', response_timeout=15, dark=True,favicon='https://operationslakedb.blob.core.windows.net/gsa-post-match/fav.png')
...
<header class="bg-[url(/images/haddad.jpg)] w-full relative">
    ...
    @ui.page('/home_tec', response_timeout=15, dark=True,favicon='https://operationslakedb.blob.core.windows.net/gsa-post-match/fav.png')
    async def home_tec():
        ui.page_title('GSA')
        #ui.add_head_html('<style>body {background-color: #81D4FA; }</style>')
        ui.add_body_html(f'''
<header class="bg-[url(/images/haddad.jpg)] w-full relative">
    ...
    <div class="container relative mx-auto flex items-center content-center align-middle h-full-mb-16">
        ...
        player_home_tec = ui.refreshable(home_tec_table)
        data = []
        #ui.markdown(f'# {prefix}').classes('mx-auto')
        #ui.markdown(f'# LIST OF POST MATCH REPORTS').classes('mx-auto')
        tournaments = set()
        ...
        for player in sorted(['SINNER', 'ZHANG', 'BERGS', 'PAVLYUCHENKOVA', 'TSITSIPAS', 'NAVARRO', 'SWIATEK', 'FILS', 'HADDAD MAIA', 'GAUFF', 'HURKACZ', 'OSAKA', 'FONSECA', 'PAUL', 'DJOKOVIC', 'COBOLLI', 'KECMANOVIC', 'ALTMAIER']):
            data.append({
                'PLAYER': player, 
                'LINK': f'https://gsapostmatch.azurewebsites.net/tec/match_new_list_tec/{player}'
                })
            # data['link'] = 'https://gsapostmatch.azurewebsites.net/gui/match_new/MEDVEDEV_Miami_2024_SF_Medvedev_Sinner'
        #ui.label(str(data))
        columns = [{'name': col, 'label': col, 'field': col, 'align': 'center'} for col in data[0].keys()]
        ...
        player_home_tec(columns=columns, data=data, table_classes="text-center w-8/12 mx-auto")
    ...
    @ui.page('/match_newtec/{match_str}', response_timeout=15, favicon='https://operationslakedb.blob.core.windows.net/gsa-post-match/fav.png')
    ...
    @ui.page('/movement_report_tec/{match_str}', response_timeout=15, favicon='https://operationslakedb.blob.core.windows.net/gsa-post-match/fav.png', dark=True)
