...
class RallyEndingDirection:
    ...
    def __init__(self, fpdf, data_repo, variant, visuals_generator):
        self.name = f"Rally ending direction page"
        self.fpdf = fpdf
        ...
        self.player2 = (
            f"{self.report_metadata['GS']} Opponent(s)"
            if "opponent" in self.player2.lower()
            else self.player2
        )
        self.fpdf.set_header_title(f"Rallies and groundstrokes")
        self.fpdf.set_header_sub_title(f"Rally ending 2-shot directions")
    ...
    def build_page_body(self):
        ...
        self.fpdf.cell(0, line_height, f"Rally ending shot: {self.player1}", align="C")
        self.fpdf.ln(line_height)
        self.fpdf.cell(10)
        self.fpdf.cell(180, line_height, f"2nd to last shot: {self.player2}", align="C")
        # self.fpdf.ln(line_height * 1.85)
        self.fpdf.set_font("Din", style="", size=10)
        self.fpdf.set_y(43.5)
        self.fpdf.set_x(15)
        self.fpdf.multi_cell_normal(175, 4, RALLY_ENDING_DIRECTION_PARAGRAPH)

        w = 96
        visual_keys = (92, 95, 93, 94, 96, 97)
        ...
        self.fpdf.cell_normal(
            0,
            line_height,
            f"*{self.report_metadata['GS']} matches with rally ending shot direction (this page) vs. {self.report_metadata['Basic']} matches with data for other visuals in the report",
            align="C",
        )

        # Gray arrow meaning
        self.fpdf.set_font(family="Din", size=9)
        self.fpdf.image(f"{CURDIRPATH}/assets/last_shot_direction.png", 17, 250, 8)
        self.fpdf.text_normal(25, 257, "Arrow indicating 2nd")
        self.fpdf.text_normal(25, 261, "to last shot direction")

        # Player info
        self.fpdf.set_font("Din Medium", size=9)
        self.fpdf.text_normal(70, 257, f"Current shot: {self.player1}")
        self.fpdf.text_normal(69.5, 261, f"Previous shot: {self.player2}")
        ...
        self.fpdf.text_normal(159, 257, "according to % of last")
        self.fpdf.text_normal(159, 261, "shot by direction")
