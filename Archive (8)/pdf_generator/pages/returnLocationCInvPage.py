...
class ReturnLocationCInvPage:
    ...
    def __init__(self, fpdf, data_repo, duplicates, visuals_generator):
        ...
        self.title = f"Return placement: {self.player1}"
        self.sub_title = f"Serve: {self.player2}"
        self.name = f"return Location C Inv {duplicates} page"

    def build_page_body(self):
        self.fpdf.set_preset("visual")
        self.fpdf.set_section(7 if self.duplicates == "1st" else 9)
        self.fpdf.add_page()
        self.fpdf.ln(7)

        self.fpdf.set_font("Din Medium", size=10)
        line_height = self.fpdf.font_size * 1.25
        self.fpdf.text(82, 38, self.title)
        self.fpdf.text(106.2, 38 + line_height, self.sub_title)
        ...
        x, y, w = 17, 65, 182
        key = 47 if self.duplicates == "1st" else 64
        ...
        self.fpdf.set_font("Din Medium", size=10)
        self.fpdf.set_text_color(0, 0, 0)
        self.fpdf.text(82, 260, self.title)
        self.fpdf.text(106.2, 264, self.sub_title)
