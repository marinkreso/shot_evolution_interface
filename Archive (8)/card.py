...
not_by_100 = ['fh_consistency_winners', 
              'fh_amount',
              'bh_amount',
'first_serve_speed',
        'first_serve_speed_deuce_w',
...
def get_data_for_matches(selected_player_name, leaderboardx, match_ids=[]):
    ...
    for k in funcs:
        ...
        if leaderboard[f'{k}_total'].sum() == 0 and k not in ['return_speed_reduction_break', 'serve_speed_reduction_pressure', 'rally_winners_per_match', 'rally_ue_per_match']:
            data_new[k] = 'NO SHOTS/RALLIES'
        elif leaderboard[f'{k}_total'].sum() < 6 and k not in ['return_speed_reduction_break', 'serve_speed_reduction_pressure', 'rally_winners_per_match', 'rally_ue_per_match']:
...
'dropshots_bh_win']:
                ...
                shot_info = 'SHOT'
                if leaderboard[f'{k}_total'].sum() > 1:
                    shot_info = 'SHOTS'
                if k in ['break_point_faced_win',
        'break_point_opportunity_win']:
                    shot_info = shot_info.replace('SHOTS', 'RALLIES').replace('SHOT', 'RALLY')
                data_new[k] = str(data_new[k]) + f" ({int(leaderboard[f'{k}_total'].sum())} {shot_info})" 
        else:
            data_new[k] = leaderboard[f'{k}_mul'].astype(float).sum() / leaderboard[f'{k}_total'].astype(float).sum()
            if k not in not_by_100:
                data_new[k] = robust_round(data_new[k] * 100)
