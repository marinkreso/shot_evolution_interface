...
async def insert_data_and_generate_link(nor, prefix, results):  
    ...
    if all(player for player in nor.players):
        ...
        import json
        with open('data_list_test.json', 'w') as f:
...
def init(fastapi_app: FastAPI) -> None:
    ...
    async def main_page_new(match_str: str):
        ...
        with open(f'matches_new2/{match_str}/movement.json') as f:
            dm = json.load(f)
        import os
        if os.path.exists(f'matches_new2/{match_str}/fonseca_return.csv'):
            dfr = pd.read_csv(f'matches_new2/{match_str}/fonseca_return.csv')
            dfr_spin = pd.read_csv(f'matches_new2/{match_str}/fonseca_spin.csv')
        match = '_'.join(match_str.split('_')[1:])
        sel_playerx = match_str.split('_')[0]
        ...
        with open('post_match_links.json') as f:
            data_match_post = json.load(f)
        
        with ui.page_sticky(position='top-left').classes('z-50'):
            ui.button(icon='arrow_back', on_click=lambda: ui.navigate.to(f'https://portal.goldensetanalytics.com/post-match-report/{data_match_post[sel_playerx]}')).props('fab')
        ...
        def report_view():
                ...
                with ui.tab_panels(tabs).classes('w-full').bind_value(selected_tab, 'number'):
                    ...
                    with ui.tab_panel('r1'):
                        ...
                        ui.label('Return Position').classes('mx-auto').tailwind('font-bold')
                        if os.path.exists(f'matches_new2/{match_str}/fonseca_return.csv'):
                            dfr_first = dfr[dfr.Serve == '1st']
                            ui.table(
                            columns=[{'name': col, 'label': col, 'field': col, 'align': 'center'} for col in dfr_first.columns],
                            rows=dfr_first.to_dict('records')
                            ).classes('mx-auto')
                            ui.label('Return Spin').classes('mx-auto').tailwind('font-bold')
                            dfr_first = dfr_spin[dfr_spin.Serve == '1st']
                            ui.table(
                            columns=[{'name': col, 'label': col, 'field': col, 'align': 'center'} for col in dfr_first.columns],
                            rows=dfr_first.to_dict('records')
                            ).classes('mx-auto')
                        
                        #ui.table(columns=dfr.columns, rows=dfr.to_dict('records'), row_key='name').classes('w-full').classes('my-table').classes('my-table-header')
                    #if sel_playerx == 'NAVARRO':
                    if True:
                        with ui.tab_panel('heat'):
                            if chosen_set_object.chosen_set == 'ALL':
                                ...
                                ui.image(images['heatmap_rallies'].replace('.png', f'_{chosen_set_object.chosen_set}.png')).classes('w-1/2').classes('mx-auto')
                                ui.image(images['heatmap_first_return'].replace('.png', f'_{chosen_set_object.chosen_set}.png')).classes('w-1/2').classes('mx-auto')
                                ui.image(images['heatmap_second_return'].replace('.png', f'_{chosen_set_object.chosen_set}.png')).classes('w-1/2').classes('mx-auto')
                                ui.image(images['heatmap_first_splus'].replace('.png', f'_{chosen_set_object.chosen_set}.png')).classes('w-1/2').classes('mx-auto')
                                ui.image(images['heatmap_second_splus'].replace('.png', f'_{chosen_set_object.chosen_set}.png')).classes('w-1/2').classes('mx-auto')
