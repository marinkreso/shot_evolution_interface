...
def tab_report(data, selected_matches, report_names, image_dir, selected_player):
    print('SELECTED MATCHES HERE', selected_matches)
    ...
    if selected_matches and selected_matches[0]:
        ...
        with ui.tab_panels(tabs, value='x1').classes('w-full'):
            with ui.tab_panel('x1').classes('mx-auto'):
                ...
                if movement_patterns_data.empty:
                    ...
                    print(movement_patterns_data, selected_matches)
...
def init(fastapi_app: FastAPI) -> None:
    ...
    async def match_new_list(prefix: str):
        ...
        with ui.row().classes('w-1/2 mx-auto'):
            sel_tournament = ui.select(label='Tournaments', options=sorted(list(tournaments)), multiple=True, on_change=on_company_change).classes('w-40').props('use-chips').props('rounded outlined').props('popup-content-style="height: 200px"')
            selected_opponents = ui.select(label='Opponents', options=sorted(list(set(opponents))), with_input=True, multiple=True, on_change=on_company_change).classes('w-40').props('use-chips').props('rounded outlined')
            selected_surfaces = ui.select(label='Surfaces', options=sorted(list(set(['hard', 'clay', 'grass']))), with_input=True, multiple=True, on_change=on_company_change).classes('w-40').props('use-chips').props('rounded outlined')
    ...
    async def movement_page(match_str: str):
        ...
        db = {'MatchData': sorted(data_movement_metadata[selected_player], key=lambda x: x['DATE'], reverse=True)}
        ...
        async def button_view(x):
            ...
            noe.report_names = ['Report']*x
            dialogs = []
            buttonopeners = []
            selected_surfaces_list = []
            selected_years_list = []
            selected_opponents_list = []
            async def on_company_change(i):
                ...
                crud_table_refreshable.refresh(rows, columns, table_classes="text-center w-full", dialog=dialog, movement_data=data, image_dir=image_dir, selected_player=selected_player, tab_report=tab_report_refreshable, noe=noe, i=i, buttonopener=buttonopeners[i])
            with ui.row().classes('mx-auto'):
                for i in range(x):
                    with ui.column():
                    ...
                    with ui.dialog() as dialog, ui.card():
                        print('WITHIN A DIALOG')
                        with ui.row().classes('w-full'):
                            selected_surfaces = ui.select(label='Surfaces', options=surfaces, multiple=True, on_change=lambda i=i: on_company_change(i)).classes('w-40').props('use-chips').props('rounded outlined')
                            selected_years = ui.select(label='Seasons', options=years, multiple=True, on_change=lambda i=i: on_company_change(i)).classes('w-40').props('use-chips').props('rounded outlined')
                            selected_opponents = ui.select(label='Opponents', options=list(set(opponents)), with_input=True, multiple=True, on_change=lambda i=i: on_company_change(i)).classes('w-40').props('use-chips').props('rounded outlined')
                        ...
                        matches = await MatchData.all(db=db)
                        rows = [
                            {
                                'SELECT': False,
                                'OPPONENT': m.OPPONENT,
                        ...
                        crud_table_refreshable(rows, columns, table_classes="text-center mx-auto w-full", dialog=dialog, movement_data=data, image_dir=image_dir, selected_player=selected_player, tab_report=tab_report_refreshable, noe=noe, i=i, buttonopener=buttonopener)                       
                    #ui.label(f'Label {i}!')
        ...
        opponents = []
        for x in await MatchData.all(db):
