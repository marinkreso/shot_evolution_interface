    ...
    with ui.header().classes('bg-black text-white p-4 shadow-md'):
        with ui.row().classes('items-center justify-between w-full'):
            ...
            with ui.row().classes('mx-auto'):
                ...
                on_players = ['SWIATEK', 'COBOLLI', 'FONSECA', 'SHELTON']
                selection = ui.select(on_players, value=prefix, on_change=lambda e: ui.navigate.to(f'https://gsapostmatch.azurewebsites.net/on/match_new_list_on/{e.value}')).props(f'color=bg-gray-800').style('font-size: 120%;').props('rounded outlined')
                
                #videoplatform = ui.button('Video Platform', on_click=lambda: ui.navigate.to(f'https://gsapostmatch.azurewebsites.net/on/shots_view_on/{prefix}')).props(f'color=bg-gray-800').style('font-size: 120%')#; text-decoration:underline')#.classes('text-lg')
                ...
                #selection = ui.select(on_players, value=prefix, on_change=lambda e: ui.navigate.to(f'https://gsapostmatch.azurewebsites.net/on/match_new_list_on/{e.value}'))
                
                postmatch = ui.button('Post Match Reports', on_click=lambda: ui.navigate.to(f'https://gsapostmatch.azurewebsites.net/on/match_new_list_on/{prefix}')).props(f'color=bg-gray-800').style('font-size: 120%;')#.classes('text-lg')
                tracking = ui.button('Match Evolution Tracker', on_click=lambda: ui.navigate.to(f'https://gsapostmatch.azurewebsites.net/on/tracking_on/{prefix}')).props(f'color=bg-gray-800').style('font-size: 120%;')#.classes('text-lg')
                shot_evolution = ui.button('Shot Evolution Tool', on_click=lambda: ui.navigate.to(f'https://gsapostmatch.azurewebsites.net/on/shot_evolution_interface_on/{prefix}')).props(f'color=bg-gray-800').style('font-size: 120%;')#.classes('text-lg')
                movement_tool = ui.button('Movement Tool', on_click=lambda: ui.navigate.to(f'https://gsapostmatch.azurewebsites.net/on/movement_report_on/{prefix}')).props(f'color=bg-gray-800').style('font-size: 120%;')#.classes('text-lg')
...
async def insert_data_and_generate_link(nor, prefix, results):  
    ...
    if all(player for player in nor.players):
        ...
        ui.navigate.to(f'https://gsapostmatch.azurewebsites.net/on/report_on2/{prefix}_{link_id}')
        #ui.navigate.to(f'http://127.0.0.1:8000/on/report_on2/{prefix}_{link_id}')
    else:
...
def init(fastapi_app: FastAPI) -> None:
    ...
    ui.run_with(
        fastapi_app,
        mount_path='/on',  # NOTE this can be omitted if you want the paths passed to @ui.page to be at the root
    ...
    @ui.page('/tracking_on/{prefix}', dark=True,favicon='https://operationslakedb.blob.core.windows.net/gsa-post-match/fav.png')
    def tracking(prefix):
    ...
    @ui.page('/match_new_list_on2/{prefix}',favicon='https://operationslakedb.blob.core.windows.net/gsa-post-match/fav.png')
    def match_new_list_on2(prefix):
        ...
        #style="position: relative; top: -10vh; height: 110vh; width: 100%; border: none;"></iframe>
    @ui.page('/points_view_on/{prefix}',favicon='https://operationslakedb.blob.core.windows.net/gsa-post-match/fav.png')
    def points_view_on(prefix):
        ...
        #style="position: relative; top: -10vh; height: 110vh; width: 100%; border: none;"></iframe>
    @ui.page('/shots_view_on/{prefix}',favicon='https://operationslakedb.blob.core.windows.net/gsa-post-match/fav.png')
    def shots_view_on(prefix):
    ...
    @ui.page('/report_on2/{prefix}_{keyx}',favicon='https://operationslakedb.blob.core.windows.net/gsa-post-match/fav.png')
    def report_on2(prefix, keyx):
        create_text_header(prefix)
        ui.add_head_html('<style>body {background-color: #081424; }</style>')
        #ui.html('''<iframe src="https://app.goldensetanalytics.com/shotsview" width="100%" height="1000px" style="clip-path: inset(10% 0 0 0); border:none;"></iframe>''').classes('w-full')
    ...
    @ui.page('/shot_evolution_interface_on/{prefix}', response_timeout=15, dark=True,favicon='https://operationslakedb.blob.core.windows.net/gsa-post-match/fav.png')
...
<header class="bg-[url(/images/footer.jpg)] w-full relative">
    ...
    <div class="container relative mx-auto flex items-center content-center align-middle h-full min-h-[200px] mt-10">
        ...
        ui.space()
        ui.space()
    @ui.page('/match_new_list_on/{prefix}', response_timeout=15, dark=True,favicon='https://operationslakedb.blob.core.windows.net/gsa-post-match/fav.png')
...
<header class="bg-[url(/images/haddad.jpg)] w-full relative">
    ...
    @ui.page('/home_on', response_timeout=15, dark=True,favicon='https://operationslakedb.blob.core.windows.net/gsa-post-match/fav.png')
    async def home_on():
        ui.page_title('GSA')
        #ui.add_head_html('<style>body {background-color: #81D4FA; }</style>')
        ui.add_body_html(f'''
<header class="bg-[url(/images/haddad.jpg)] w-full relative">
    ...
    <div class="container relative mx-auto flex items-center content-center align-middle h-full-mb-16">
        ...
        player_home_on = ui.refreshable(home_on_table)
        data = []
        #ui.markdown(f'# {prefix}').classes('mx-auto')
        #ui.markdown(f'# LIST OF POST MATCH REPORTS').classes('mx-auto')
        tournaments = set()
        ...
        for player in sorted(['SINNER', 'ZHANG', 'BERGS', 'PAVLYUCHENKOVA', 'TSITSIPAS', 'NAVARRO', 'SWIATEK', 'FILS', 'HADDAD MAIA', 'GAUFF', 'HURKACZ', 'OSAKA', 'FONSECA', 'PAUL', 'DJOKOVIC', 'COBOLLI', 'KECMANOVIC', 'ALTMAIER']):
            data.append({
                'PLAYER': player, 
                'LINK': f'https://gsapostmatch.azurewebsites.net/on/match_new_list_on/{player}'
                })
            # data['link'] = 'https://gsapostmatch.azurewebsites.net/gui/match_new/MEDVEDEV_Miami_2024_SF_Medvedev_Sinner'
        #ui.label(str(data))
        columns = [{'name': col, 'label': col, 'field': col, 'align': 'center'} for col in data[0].keys()]
        ...
        player_home_on(columns=columns, data=data, table_classes="text-center w-8/12 mx-auto")
    ...
    @ui.page('/match_newon/{match_str}', response_timeout=15, favicon='https://operationslakedb.blob.core.windows.net/gsa-post-match/fav.png')
    ...
    @ui.page('/movement_report_on/{match_str}', response_timeout=15, favicon='https://operationslakedb.blob.core.windows.net/gsa-post-match/fav.png', dark=True)
