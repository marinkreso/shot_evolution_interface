...
#leaderboard = pd.read_sql('he_leaderboard_wta', engine)
def main5(data
        ):
    ...
    #leaderboard = pd.read_csv('leaderboard_mph.csv')
    ...
    datas = []
    for d in data:
        if 'multi' in d and d['multi']:
            _,multi = get_data_for_player_best_matches(d['player'], leaderboard, d['multi'])
            datas.append((multi, 'REFERENCE', 'PLAYERS'))
        else:
            matches = list(leaderboard[(leaderboard.player_name == d['player']) & (leaderboard.match_id != 'ALL')].match_id.unique())
            if 'matches' in d:
                matches = [x for x in matches if x in d['matches']]
            if 'opponent' in d:
                matches = [x for x in matches if d['opponent'].lower() in x.lower()]
            if 'tournaments' in d:
                matches = [x for x in matches if any(tournament.lower() in x.lower() for tournament in d['tournaments'])]
                if 'year' in d:
                    matches2 = [x for x in matches if d['year'] in x]
                print('THESE ARE: ', matches2)
                if 'year' in d:
                    if 'name' in d:
                        datas.append((get_data_for_matches([d['player']], leaderboard, [x for x in matches if d['year'] in x]), d['player'], d['name']))
                    else:
                        datas.append((get_data_for_matches([d['player']], leaderboard, [x for x in matches if d['year'] in x]), d['player'], d['year']+ ' ' + ','.join(d['tournaments']).replace('_', ' ')))
                else:
                    if 'name' in d:
                        datas.append((get_data_for_matches([d['player']], leaderboard, [x for x in matches]), d['player'], d['name']))
                    else:
                        datas.append((get_data_for_matches([d['player']], leaderboard, [x for x in matches]), d['player'], ','.join(d['tournaments']).replace('_', ' ')))
                ...
                if 'year' in d:
                    if 'name' in d:
                        ...
                        datas.append((get_data_for_matches([d['player']], leaderboard, [x for x in matches if d['year'] in x]), d['player'], d['year']))
                else:
                    if 'name' in d:
                        datas.append((get_data_for_matches([d['player']], leaderboard, [x for x in matches]), d['player'], d['name']))
                    else:
                        datas.append((get_data_for_matches([d['player']], leaderboard, [x for x in matches]), d['player'], ''))
...
'fh_cc_spin_winners']:
    ...
    with open('datas_example.json', 'w') as f:
        import json
        json.dump(datas, f, indent=4)
    return pretty_dict, datas, data_order