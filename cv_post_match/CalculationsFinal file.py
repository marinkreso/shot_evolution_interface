...
class Calculations:
    ...
    def __init__(self, player_name, df, df_serve, is_leftie):
        ...
        Calculations.calculate_all(self, all_matches_df, all_matches_df_serve, 'ALL')
        # calculate per match statistics
        Calculations.calculate_matches(self, all_matches_df, all_matches_df_serve)

    def calculate_all(self, df, df_serve, match_id):
...
'first_return_win_percentage_break'
        ...
        Calculations.results_df = \
            pd.concat([Calculations.results_df, pd.DataFrame(new_row, index=[0])], ignore_index=True)


    # groups by match and calls calculate_all for each match
    def calculate_matches(self, all_matches_df, all_matches_df_serve):
        ...
        for match_id, match_group in grouped_matches_df:
            for match_id_serve, match_group_serve in grouped_matches_df_serve:
                if match_id == match_id_serve:
                    Calculations.calculate_all(self, match_group, match_group_serve, match_id)
    ...
    def first_return_ad_fh_speed(self, df):
        df_returns = df[(df.PLAYER_HIT == Calculations.PLAYER_NAME) & (df.shot_no == 2) & (df.is_shot_in == 1)
                        & (df.serve_number == 1) & (df.serve_deuce_or_ad == 'ad') & (df.shot_type == 'F')]
    ...
    def first_return_ad_bh_speed(self, df):
        df_returns = df[(df.PLAYER_HIT == Calculations.PLAYER_NAME) & (df.shot_no == 2) & (df.is_shot_in == 1)
                        & (df.serve_number == 1) & (df.serve_deuce_or_ad == 'ad') & (df.shot_type == 'B')]
    ...
    def second_return_ad_fh_speed(self, df):
        df_returns = df[(df.PLAYER_HIT == Calculations.PLAYER_NAME) & (df.shot_no == 2) & (df.is_shot_in == 1)
                        & (df.serve_number == 2) & (df.serve_deuce_or_ad == 'ad') & (df.shot_type == 'F')]
    ...
    def second_return_ad_bh_speed(self, df):
        df_returns = df[(df.PLAYER_HIT == Calculations.PLAYER_NAME) & (df.shot_no == 2) & (df.is_shot_in == 1)
                        & (df.serve_number == 2) & (df.serve_deuce_or_ad == 'ad') & (df.shot_type == 'B')]
    ...
    def approach_to_bh_win_perc(self, df):
        ...
        if total_points == 0:
            return 0, 0
        return round(won_points / total_points, 2), total_points
    def fh_amount(self, df):
        # Consistency
        df_shots = df[(df.PLAYER_HIT == Calculations.PLAYER_NAME) & (df.shot_no >= 4) & (df.CONTACT_X_abs > 9)]
        
        fh_shots = df_shots[df_shots.shot_type == 'F']
        bh_shots = df_shots[df_shots.shot_type == 'B']

        
        total = len(fh_shots) + len(bh_shots)
        if total == 0:
            return None
        return round(len(fh_shots) / total * 100, 2), total
    
    def bh_amount(self, df):
        # Consistency
        df_shots = df[(df.PLAYER_HIT == Calculations.PLAYER_NAME) & (df.shot_no >= 4) & (df.CONTACT_X_abs > 9)]
        ...
        if total == 0:
            return None
        return round(len(bh_shots) / total * 100, 2), total
    ...
    def on_the_run_fh_percent(self, df):
        ...
        df_run_fh = df[(df['PLAYER_HIT'] == selected_player_name) & (df['SPIN'] != 3)&(df['CONTACT_X_abs'] > 12)&(df['CONTACT_Y_mirrored']*leftie_multiplier < -1.5) & (df['CONTACT_Y_mirrored'].shift(2)*leftie_multiplier > 0) & (df['shot_type'] == 'F')
    ...
    def on_the_run_bh_percent(self, df):
        ...
        selected_player_name = Calculations.PLAYER_NAME
        df = df[(df.shot_no  >= 4) & (df.PLAYER_HIT == selected_player_name)]
        df_run_fh = df[(df['PLAYER_HIT'] == selected_player_name) & (df['SPIN'] != 3)&(df['CONTACT_X_abs'] > 12)&(df['CONTACT_Y_mirrored']*leftie_multiplier > 2) & (df['CONTACT_Y_mirrored'].shift(2)*leftie_multiplier < 0) & (df['shot_type'] == 'B')
              & (df['SPEED'] >75) & (df['shot_no'] > 3) & (df['sel_player_location_at_shot_y_abs_diff'] > 4)]
        df_run_fh = df_run_fh[df_run_fh.CONTACT_Y_mirrored*leftie_multiplier > 2]
